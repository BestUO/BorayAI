...
public:
    void dr_cb (RdKafka::Message &message)
    {
        ...
        LOG(INFO) << "Message delivery for (" << message.len() << " bytes): " << status_name << ": " << message.errstr();
        if (message.key())
...
public:
    ...
    bool CreateProducer(std::string brokers);
...
private:
    ...
    std::vector<std::shared_ptr<RdKafka::Producer>> _producers;
    std::mutex _mutex;
    ...
    void ConsumerAddThread(std::shared_ptr<RdKafka::KafkaConsumer> consumerptr, int batch_size, Function f)
    {
        while (!_stopconsumers)
        {
            ...
            LOG(INFO) << "Accumulated " << msgs.size() << " messages:";
            for (auto &msg : msgs) 
            {
                LOG(INFO) << " Message in " << msg->topic_name() << " [" << msg->partition() << "] at offset " << msg->offset();
    ...
    std::shared_ptr<RdKafka::Producer> GetRandomProducer();
};